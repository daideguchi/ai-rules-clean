                    ...
                    if detail_response.status_code == 200:
                        ...
                        for node_name, node_executions in run_data.items():
                            ...
                            if node_executions:
                                ...
                                if error:
                                    print(
                                        f"            ❌ エラー: {error.get('message', 'N/A')}"
                                    )
                                    if "cause" in error:
                                        print(
                                            f"            原因: {error.get('cause', 'N/A')}"
                                        )
                                else:
                                    print("            ✅ 正常")
                            else:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data:
                ...
                for item in data:
                    print(
                        f"      ID: {item.get('id')}, Success: {item.get('task_success')}"
                    )
                return True
            else:
                print("   ❌ データなし")
...
def main():
    ...
    if executions:
        ...
        if status == "error":
            print("  🚨 **重大問題**: n8nワークフロー内部でエラーが発生")
            print("  📋 **統合状況**: 完全に機能していません")
            print("  💡 **対応必要**: ワークフロー設定の修正が必要")
        elif status == "success":
            if supabase_success:
                print("  ✅ **完全成功**: n8n→Supabase統合が正常動作")
            else:
                print("  ⚠️ **部分成功**: n8nは動作するがSupabase反映に問題")
        else:
            print("  ❓ **不明状態**: 実行ステータスが特定できません")
    else:
        print("  ❌ **実行確認失敗**: 実行履歴を取得できませんでした")
