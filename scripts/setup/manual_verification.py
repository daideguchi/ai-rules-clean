            ...
            if detail_response.status_code == 200:
                ...
                if exec_response.status_code == 200:
                    ...
                    for exec_data in executions[:2]:
                        print(f"      - {exec_data.get('status', 'N/A')} ({exec_data.get('mode', 'N/A')})")
...
def manual_instructions():
    ...
    print(f"1. **n8n UIでワークフロー確認**:")
    print(f"   https://dd1107.app.n8n.cloud")
    
    print(f"\n2. **「Working Claude AI v1752496422」を開く**")
    
    print(f"\n3. **右上のトグルスイッチ確認**:")
    print(f"   - 現在: Inactive（グレー）の可能性")
    print(f"   - 手動でActive（緑）に切り替え")
    
    print(f"\n4. **確認後、以下を実行**:")
    print(f"   curl -X POST 'https://dd1107.app.n8n.cloud/webhook/working-claude-1752496422' \\")
    print(f"     -H 'Content-Type: application/json' \\")
    print(f"     -d '{{\"session_id\":\"manual_ui_test\",\"success\":true,\"execution_time\":1.0,\"tools_used\":\"manual\",\"error_count\":0,\"thinking_tag_used\":true,\"todo_tracking\":true,\"task_complexity\":\"simple\",\"learning_score\":5}}'")
    
    print(f"\n5. **成功確認**:")
    print(f"   - 応答: {{\"message\":\"Workflow was started\"}}")
