...
def use_existing_workflow():
    ...
    # 現在の状態確認
    detail_response = requests.get(f"{base_url}/api/v1/workflows/{existing_workflow_id}", headers=headers)
    ...
    if detail_response.status_code == 200:
        ...
        if not current_active:
            # アクティブ化試行
            print("⚡ アクティブ化試行...")
            activate_response = requests.post(f"{base_url}/api/v1/workflows/{existing_workflow_id}/activate", headers=headers)
