...
class N8nWorkflowUpdater:
    ...
    def list_workflows(self):
        ...
        try:
            ...
            if len(workflows) == 0:
                ...
                print(f"  - 全レスポンス: {data}")
                print("  - 可能な原因: 無料プラン制限、アクセス権限不足、ワークフロー未作成")
    ...
    def create_supabase_http_node(self, workflow):
        ...
        # 次のノードIDを生成
        existing_ids = [node.get('id', '') for node in workflow.get('nodes', [])]
        node_id = f"supabase-http-{int(datetime.now().timestamp())}"
        
        # 適切な位置を計算
        max_x = max([node.get('position', [0, 0])[0] for node in workflow.get('nodes', [])], default=0)
    ...
    def update_claude_performance_workflow(self):
        ...
        supabase_nodes = [node for node in workflow.get('nodes', []) 
...
def main():
    ...
    if success:
        ...
        print("\n❌ n8nワークフロー自動更新失敗")
        print("💡 手動設定手順: /Users/dd/Desktop/1_dev/coding-rule2/config/n8n/WORKFLOW_FIX_INSTRUCTIONS.md")
