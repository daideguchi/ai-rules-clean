...
def detailed_workflow_check(workflow, headers, base_url):
    ...
    # 詳細情報取得
    detail_response = requests.get(f"{base_url}/api/v1/workflows/{workflow_id}", headers=headers)
...
def multiple_activation_attempts(workflow_id, headers, base_url):
    ...
    methods = [
        ("POST /activate", lambda: requests.post(f"{base_url}/api/v1/workflows/{workflow_id}/activate", headers=headers)),
        ("PUT active=true", lambda: requests.put(f"{base_url}/api/v1/workflows/{workflow_id}", headers=headers, json={"active": True})),
        ("PATCH active=true", lambda: requests.patch(f"{base_url}/api/v1/workflows/{workflow_id}", headers=headers, json={"active": True}))
    ]
    ...
    for i, (method_name, method_func) in enumerate(methods, 1):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # 確認
                time.sleep(2)
                check_response = requests.get(f"{base_url}/api/v1/workflows/{workflow_id}", headers=headers)
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if check_response.status_code == 200 and check_response.json():
                    print(f"   ✅ Supabaseデータも確認成功")
                    return True
                else:
                    print(f"   ⚠️ Webhookは成功したがSupabaseデータなし")
                    return False
            else:
