...
class CloudflareBypassImport:
    ...
    def _post_with_form_encoding(self, workflow_data: Dict) -> Optional[requests.Response]:
        """Form-Encoding経由POST"""
    ...
    def _post_with_chunked_encoding(self, workflow_data: Dict) -> Optional[requests.Response]:
        ...
        try:
            ...
            url = f"{self.n8n_base_url}{self.working_endpoint}"
            response = self.session.post(url, data=chunked_data(), headers=headers, timeout=30)
    ...
    def _post_via_put_method(self, workflow_data: Dict) -> Optional[requests.Response]:
        ...
        try:
            ...
            url = f"{self.n8n_base_url}{self.working_endpoint}"
            response = self.session.put(url, json=workflow_data, headers=headers, timeout=30)
            ...
            for method in bypass_methods:
                ...
                if response and response.status_code in [200, 201]:
                    try:
                        ...
                        print(f"✅ 回避インポート成功: {workflow_name} (ID: {workflow_id})")
