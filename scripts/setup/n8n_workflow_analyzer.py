...
class N8nWorkflowAnalyzer:
            ...
            for node in nodes:
                if node.get('type') == 'n8n-nodes-base.webhook':
                    ...
                    if 'claude-performance' in path.lower():
                        print(f"  ✅ 発見: {wf['name']} (path: {path}, active: {wf.get('active')})")
    ...
    def check_supabase_integration(self, nodes):
        ...
        return len(supabase_nodes) > 0 or any('supabase' in node.get('parameters', {}).get('url', '') for node in http_nodes)
    ...
    def generate_fix_recommendation(self, workflow, has_supabase_integration):
        ...
        nodes = workflow.get('nodes', [])
        postgres_nodes = [n for n in nodes if n.get('type') == 'n8n-nodes-base.postgres']
        if postgres_nodes:
            print("  ⚠️ PostgreSQLノードが検出されました")
            print("  📋 問題: Supabase REST APIとPostgreSQLノードは競合する可能性があります")
