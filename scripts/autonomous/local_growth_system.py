...
class LocalAutonomousGrowthSystem:
        ...
        def receive_performance_data():
            ...
            try:
                ...
                if learning_result["should_evolve"]:
                    threading.Thread(
                        target=self._trigger_immediate_evolution, daemon=True
                    ).start()
        ...
        with sqlite3.connect(self.db_path) as conn:
            for pattern in patterns:
                ...
                if result:
                    ...
                    """,
                        (datetime.now().isoformat(), "behavioral", pattern, 1.0, 1),
                    )
    ...
    def _analyze_top_patterns(self) -> Dict:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            success_patterns = [
                {"pattern": row[0], "frequency": row[1]} for row in cursor.fetchall()
            ]
            ...
            return {"success": success_patterns, "failure": failure_patterns}
    ...
    def _evolve_claude_md(self, patterns: Dict) -> Dict:
        ...
        if evolution_rules:
            ...
            # バックアップ作成
            backup_path = (
                self.claude_md_path.parent / f"CLAUDE.md.backup.{int(time.time())}"
            )
    ...
    def _apply_evolution_rules(self, content: str, rules: List[Dict]) -> str:
        ...
        if "## 🧬 Auto-Evolution Rules" in content:
            ...
            content = re.sub(
                pattern, evolution_section.strip(), content, flags=re.DOTALL
            )
        else:
            # 新セクション追加
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            return {
                ...
                "latest_evolution": {
                    ...
                    "reason": latest_evolution[2]
                    if latest_evolution
                    else "No evolution yet",
                }
                if latest_evolution
                else None,
    ...
    def start_server(self):
        ...
        print(
            f"📍 Webhook URL: http://localhost:{self.port}/webhook/claude-performance"
        )
