...
class LocalAutonomousGrowth:
    ...
    def analyze_patterns(self):
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            # 結果表示
            print(f"\n✅ 成功パターン頻度:")
            for pattern, data in sorted(all_success_patterns.items(), key=lambda x: x[1]['count'], reverse=True):
                rate = (data['success_count'] / data['count'] * 100) if data['count'] > 0 else 0
                print(f"  - {pattern}: {data['count']}回 (成功率: {rate:.1f}%)")
            
            print(f"\n❌ 失敗パターン頻度:")
            for pattern, data in sorted(all_failure_patterns.items(), key=lambda x: x[1]['count'], reverse=True):
                rate = (data['failure_count'] / data['count'] * 100) if data['count'] > 0 else 0
        ...
        for i, rec in enumerate(recommendations, 1):
            ...
            with sqlite3.connect(self.db_path) as conn:
                conn.execute("""
                    ...
                    VALUES (?, ?, ?, ?)
                """, (datetime.now().isoformat(), 'improvement', f'recommendation_{i}', rec))
