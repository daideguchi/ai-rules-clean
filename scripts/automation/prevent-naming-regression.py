                ...
                if result.returncode == 0:
                    for line in result.stdout.strip().split('\n'):
                        if line:
                            violations.append({
                                "term": term,
                                "location": line.split(':', 1)[0] if ':' in line else line,
                                "content": line,
                                "suggested_fix": self.preferred_terms.get(term, "ルール確認")
                            })
...
"""
        ...
        for violation in violations:
            report_content += f"- **{violation['term']}** in `{violation['location']}`\n"
...
def main():
    ...
    parser.add_argument("--setup-hook", action="store_true", help="Pre-commitフック設置")
