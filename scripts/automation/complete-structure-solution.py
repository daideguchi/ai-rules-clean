...
def check_current_status():
    ...
    # 命名規則違反確認
    naming_violations = []
    for root, dirs, files in project_root.rglob('*'):
        for file in files:
    ...
    if old_marketing.exists():
        print(f"📁 古いマーケティングディレクトリ存在")
        print(f"   ❌ scripts/n8n_marketing/ → scripts/marketing/ 要修正")
    
    total_issues = len(root_violations) + len(naming_violations) + (1 if old_marketing.exists() else 0)
    print(f"\n📊 総問題数: {total_issues}個")
