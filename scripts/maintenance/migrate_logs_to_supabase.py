...
class LogMigration:
    ...
    def parse_log_entry(self, log_path: Path, line: str) -> Optional[Dict]:
        ...
        try:
            ...
            # エラーかどうかを判定
            is_error = any(keyword in line.lower() for keyword in ["error", "failed", "exception", "traceback"])
            
            # 成功かどうかを判定
            is_success = any(keyword in line.lower() for keyword in ["success", "completed", "done", "✅"])
    ...
    def insert_batch(self, batch_data: List[Dict]) -> bool:
        """バッチデータをSupabaseに挿入"""
        try:
            result = self.supabase.table("ai_performance_log").insert(batch_data).execute()
            return True
        except Exception as e:
            print(f"❌ バッチ挿入失敗: {e}")
            return False
