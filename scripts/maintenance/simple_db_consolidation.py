...
def consolidate_databases():
    ...
    with sqlite3.connect(core_db_path) as conn:
        ...
        """,
            (
                "forever_ledger,user_prompts,autonomous_growth,ai_growth",
...
def test_new_architecture():
    ...
    try:
        with sqlite3.connect(db_dir / "core.db") as conn:
            ...
            cursor = conn.execute(
                "SELECT name FROM org.sqlite_master WHERE type='table'"
            )
            org_tables = [row[0] for row in cursor.fetchall()]

            cursor = conn.execute(
                "SELECT name FROM enforce.sqlite_master WHERE type='table'"
            )
...
def main():
    ...
    print("CLAUDE.md にアーキテクチャ詳細を記録済み")
