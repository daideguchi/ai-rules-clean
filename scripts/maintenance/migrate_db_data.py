        ...
        with sqlite3.connect(target_path) as target_conn:
            ...
            for source_db_path, description in source_list:
                ...
                try:
                    ...
                    target_conn.execute(
                        f"ATTACH DATABASE '{full_source_path}' AS {attach_name}"
                    )

                    # ソースDBのテーブル一覧取得
                    cursor = target_conn.execute(
                        f"SELECT name FROM {attach_name}.sqlite_master WHERE type='table' AND name NOT LIKE 'sqlite_%'"
                    )
                    ...
                    for table_name in source_tables:
                        # テーブル構造をコピー
                        cursor = target_conn.execute(
                            f"SELECT sql FROM {attach_name}.sqlite_master WHERE name='{table_name}'"
                        )
                        ...
                        if create_sql:
                            # テーブル作成（存在しない場合のみ）
                            target_conn.execute(
                                f"CREATE TABLE IF NOT EXISTS {table_name}_temp AS SELECT * FROM {attach_name}.{table_name} WHERE 1=0"
                            )

                            # データ挿入
                            cursor = target_conn.execute(
                                f"SELECT COUNT(*) FROM {attach_name}.{table_name}"
                            )
                            row_count = cursor.fetchone()[0]

                            if row_count > 0:
                                target_conn.execute(
                                    f"INSERT OR IGNORE INTO {table_name}_temp SELECT * FROM {attach_name}.{table_name}"
                                )
    ...
    for db_file in ["core.db", "ai_organization.db", "enforcement.db"]:
        ...
        try:
            with sqlite3.connect(db_path) as conn:
                ...
                print(
                    f"  ✅ {db_file}: {len(tables)}テーブル, {total_rows}行, {size_kb}KB"
                )
...
def main():
    ...
    print("\n🎯 データベース統合・移行完了!")
    print("o3ベストプラクティス準拠の3DB統一アーキテクチャが稼働中")
