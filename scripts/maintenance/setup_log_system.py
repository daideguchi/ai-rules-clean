    ...
    def test_logging(self) -> bool:
        ...
        try:
            with self.connection.cursor() as cursor:
                ...
                print(f"✅ テストログ記録完了 (performance_id: {log_id}, event_id: {event_id})")
                return True
...
def main():
    ...
    try:
        if log_system.setup_logging_system():
            ...
            if stats:
                print("\n📊 ログシステム統計:")
                print(f"  Performance Logs: {stats['performance_log']['total_entries']}件")
                print(f"  System Events: {stats['system_events']['total_events']}件")
                print(f"  Unique Sessions: {stats['performance_log']['unique_sessions']}件")
