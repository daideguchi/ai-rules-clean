...
class DatabaseConsolidation:
    def __init__(self):
        ...
        self.backup_dir = self.project_root / "runtime" / "db_backups" / datetime.now().strftime("%Y%m%d_%H%M%S")
    ...
    def create_consolidated_db(self, target_name, config):
        ...
        with sqlite3.connect(target_path) as target_conn:
            ...
            for source_db_path in config['source_dbs']:
                ...
                # ソースDBをアタッチ
                target_conn.execute(f"ATTACH DATABASE '{full_source_path}' AS source_db")
                ...
                try:
                    ...
                    for table_name, table_info in tables.items():
                        ...
                        # テーブル作成（存在確認）
                        target_conn.execute(f"CREATE TABLE IF NOT EXISTS {table_name} AS SELECT * FROM source_db.{table_name} WHERE 1=0")
                        
                        # データ挿入（重複回避）
                        target_conn.execute(f"INSERT OR IGNORE INTO {table_name} SELECT * FROM source_db.{table_name}")
                        ...
                        target_conn.execute("""
                            ...
                            VALUES (?, ?, ?, ?, ?)
                        """, (source_db_path, table_name, table_name, datetime.now().isoformat(), table_info['rows']))
        ...
        for db_name, config in self.target_dbs.items():
            ...
            if db_path.exists():
                ...
                report["databases"][db_name] = {
                    ...
                    "table_details": {name: {"rows": info['rows'], "columns": info['columns']} 
    ...
    def consolidate_all(self):
        ...
        for db_name, info in report["databases"].items():
            print(f"  - {db_name}: {info['tables']}テーブル, {info['total_rows']}行, {info['file_size_kb']}KB")
        
        return report

def main():
    consolidation = DatabaseConsolidation()
    report = consolidation.consolidate_all()
