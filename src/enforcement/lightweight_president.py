...
class LightweightPresident:
    ...
    def full_validation(self) -> PresidentStatus:
        ...
        try:
            ...
            # Check if declaration is still valid (1 hour expiry)
            if declared_at:
                expires_at = declared_at + timedelta(hours=1)
    ...
    def _quick_file_check(self) -> PresidentStatus:
        ...
        try:
            ...
            # Check if file is recent enough (1 hour)
            if datetime.now() - mod_time > timedelta(hours=1):
                return PresidentStatus(
                    False, mod_time, None, None, "expired", datetime.now()
                )

            # File is recent - assume valid for quick check
            expires_at = mod_time + timedelta(hours=1)
            return PresidentStatus(
                True, mod_time, expires_at, "quick", "2.0", datetime.now()
            )
    ...
    def _is_cache_valid(self) -> bool:
        ...
        with self._cache_lock:
            ...
            # Check if status itself is expired
            if (
                self._cached_status.expires_at
                and datetime.now() > self._cached_status.expires_at
            ):
                return False
    ...
    def get_detailed_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "expires_at": full_status.expires_at.isoformat()
            if full_status.expires_at
            else None,
