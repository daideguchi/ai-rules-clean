...
class CSACompleteSystemO3:
    ...
    def enhanced_context_search_v2(
        ...
        try:
            ...
            # o3統合検索クエリ（プロジェクト別 + 階層化）
            cur.execute(
                f"""
                SELECT
                    id, timestamp, source, event_type, content, metadata, importance_level,
                    project_name, file_tier, retention_category,
                    1 - (vector_embedding <=> %s::vector) as similarity
                FROM context_stream_partitioned
                WHERE vector_embedding IS NOT NULL
                  AND project_name = '{self.project_root.name}'
                  AND timestamp >= NOW() - INTERVAL '{self.db_retention_days} days'
                  {importance_condition}
                  AND (
            ...
            return {
                ...
                "search_metadata": {
                    ...
                    "project_name": self.project_root.name,
                    "search_algorithm": "o3_project_tiered_semantic",
                },
            }
...
def main():
    ...
    print("🌊 o3統合CSA文脈システム完全実装開始")

    try:
        csa_system = CSACompleteSystemO3(
    ...
    print(f"🏗️ プロジェクト: {csa_system.project_root.name}")
    print(f"💾 データベース: {csa_system.db_config['database']}")

    # 1. o3推奨プロジェクト別パーティション実装
    print("\n1️⃣ o3推奨プロジェクト別パーティション実装")
    ...
    if partition_result["status"] == "success":
        print(f"   プロジェクト: {partition_result['project_name']}")
        print(f"   データベース: {partition_result['database']}")
        print(f"   パーティション: {partition_result['partitions_created']}")
    else:
        print(f"   エラー: {partition_result.get('message', 'Unknown error')}")
        return

    # 2. o3統合CSAデータ蓄積加速
    print("\n2️⃣ o3統合CSAデータ蓄積加速処理")
    ...
    if accumulation_result["status"] == "success":
        print(f"   発見ファイル数: {accumulation_result['total_files_discovered']}")
        ...
        print(
            f"   ファイル当たり平均イベント: {accumulation_result['events_per_file_avg']}"
        )
        print(f"   プロジェクト: {accumulation_result['project_name']}")
        print(f"   データベース: {accumulation_result['database']}")

        # ファイルタイプ別統計
        print("\n   ファイルタイプ別統計:")
        for file_type, count in accumulation_result["file_type_stats"].items():
            print(f"     📂 {file_type}: {count}ファイル")
    else:
        print(f"   エラー: {accumulation_result.get('error', 'Unknown error')}")
        return
    ...
    for query, importance_filter in test_queries:
        search_result = csa_system.enhanced_context_search_v2(
            query, importance_filter, limit=5
        )
        ...
        if search_result["status"] == "success":
            ...
            for category, events in categorized.items():
                if events:
                    ...
                    for event in events[:1]:  # 最初の1件のみ表示
                        print(
    ...
    print(f"   🏗️ プロジェクト: {csa_system.project_root.name}")
    print(f"   💾 データベース: {csa_system.db_config['database']}")
    print(f"   📈 容量上限: {csa_system.max_local_storage_mb}MB")
    print(f"   🔥 ホット保持期間: {csa_system.preserve_recent_days}日")
    print(f"   🌡️ ウォーム保持期間: {csa_system.db_retention_days}日")
    print(
        f"   🛡️ 学習データ保護: {'有効' if csa_system.learning_data_protection else '無効'}"
    )

    print("\n✅ o3統合CSA文脈システム完全実装完了")
    print(
        "📍 プロジェクト別設定 + 8GB容量統合 + 階層化保護 + 全ファイル解析による大量文脈データ蓄積"
    )

    print("\n📖 使用方法:")
    print("   プロジェクト指定: python csa_complete_system_o3.py --project [path]")
    print("   設定指定: python csa_complete_system_o3.py --config [config.json]")
