...
class ErrorInfo:
    ...
    def should_fail_fast(self) -> bool:
        """即座に失敗すべきかチェック"""
        return (
            self.severity in [ErrorSeverity.FATAL]
...
class SystemErrorHandler(ErrorHandler):
    ...
    def handle(self, error_info: ErrorInfo) -> bool:
        if error_info.severity == ErrorSeverity.FATAL:
            return False
        return error_info.can_retry() and error_info.severity in [
            ErrorSeverity.LOW,
            ErrorSeverity.MEDIUM,
        ]
...
class ErrorStatistics:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "errors_by_severity": {
                k.value: v for k, v in self.errors_by_severity.items()
            },
            "errors_by_category": {
                k.value: v for k, v in self.errors_by_category.items()
            },
...
class ErrorHandlingSystem:
    ...
    def handle_error_with_fallback(
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if attempt < max_retries:
                    # リトライの場合はログのみ
                    self.logger.warning(
                        f"Operation failed, retrying ({attempt + 1}/{max_retries}): {e}"
                    )
                    continue
                else:
                    # 最終試行失敗時はエラーハンドリング実行
                    self.handle_error(
                        e, context, ErrorSeverity.HIGH, ErrorCategory.BUSINESS_LOGIC
                    )
                    break
        ...
        try:
            ...
            # フォールバックも失敗
            self.handle_error(
                fallback_error, context, ErrorSeverity.CRITICAL, ErrorCategory.SYSTEM
            )
    ...
    def get_recent_errors(self, limit: int = 10) -> List[ErrorInfo]:
        ...
        with self._lock:
            return (
                self.error_history[-limit:] if limit > 0 else self.error_history.copy()
            )
    ...
    def _attempt_recovery(self, error_info: ErrorInfo) -> Optional[Any]:
        ...
        if not handler.handle(error_info):
            self.logger.info(
                f"Handler declined to process error: {error_info.error_id}"
            )
            return None
        ...
        for action in recovery_actions:
            try:
                ...
                self.logger.error(
                    f"Recovery action failed: {action.action_name}, Error: {recovery_error}"
                )
                continue

        self.logger.warning(
            f"All recovery actions failed for error: {error_info.error_id}"
        )
        return None
    ...
    def _log_error(self, error_info: ErrorInfo) -> None:
        ...
        if error_info.traceback_info and error_info.severity in [
            ErrorSeverity.HIGH,
    ...
    def _persist_error(self, error_info: ErrorInfo) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"Failed to persist error {error_info.error_id}: {persist_error}"
            )
