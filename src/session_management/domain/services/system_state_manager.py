...
from ..entities.session_state import SessionState, ComponentState
...
class StateChangeEvent:
    ...
    @classmethod
    def component_updated(cls,
                         component_name: str,
                         new_status: str,
                         session_id: Optional[str] = None,
                         timestamp: Optional[datetime] = None) -> 'StateChangeEvent':
        """コンポーネント更新イベント作成"""
        return cls(
...
class DependencyTracker:
    ...
    def register_dependency(self,
                          ...
                          command_dependencies: Optional[List[str]] = None) -> None:
        """新しい依存関係を登録"""
...
class SystemStateManager:
    ...
    def __init__(self,
        ...
        self.dependency_tracker = dependency_tracker or DependencyTracker()
        self.state_hash_calculator = state_hash_calculator or StateHashCalculator(self.dependency_tracker)
    ...
    def update_component_state(self,
                             ...
                             component_state: ComponentState) -> SessionState:
        """コンポーネント状態更新"""
    ...
    def add_check_result(self,
                        session_state: SessionState,
                        check_result: CheckResult) -> SessionState:
        """確認結果追加"""
    ...
    def handle_command_execution(self, command: str) -> None:
        """コマンド実行処理"""
        affected_checks = self.dependency_tracker.get_affected_checks_by_command(command)
    ...
    def generate_cache_key(self, check_name: str) -> StateAwareCacheKey:
        ...
        dependencies_hash = self.state_hash_calculator.calculate_dependencies_hash(check_name)
    ...
    def validate_state_integrity(self, session_state: SessionState) -> Dict[str, Any]:
        ...
        for name, component in session_state.component_states.items():
            if component.component_name != name:
                validation_results["is_valid"] = False
                validation_results["issues"].append(f"Component name mismatch: {name} vs {component.component_name}")
        
        # 確認結果の整合性チェック
        recent_failures = [
            result for result in session_state.check_history[-10:]  # 最新10件
