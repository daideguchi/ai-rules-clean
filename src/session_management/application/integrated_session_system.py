...
from ..domain.entities.check_result import CheckResult
from ..domain.entities.session_state import ComponentState
from ..domain.entities.system_status import ComponentHealth, HealthStatus, SystemStatus
...
from ..infrastructure.cache.event_driven_cache import EventDrivenCache
...
class IntegratedEventPublisher(EventPublisher):
    ...
    def publish(self, event: Union[StateChangeEvent, Any]) -> None:
        ...
        if hasattr(event, "change_type"):
            self.logger.debug(
                f"Publishing StateChange event: {event.change_type.value}"
            )
        # CacheEventの場合
...
class IntegratedSessionSystem:
    ...
    def __init__(self, config: SessionConfiguration):
        ...
        self.error_system = ErrorHandlingSystem(
            logger=self.logger,
            persistence_path=config.persistence_path / "errors"
            if config.persistence_path
            else None,
        )

        # セッション状態
        self.session_state = self.state_manager.initialize_session_state(
            config.session_id
        )
        ...
        # 初期化完了ログ
        self.logger.info(
            f"IntegratedSessionSystem initialized for session: {config.session_id}"
        )
        ...
        try:
            if fallback:
                return self.error_system.handle_error_with_fallback(
                    operation=operation, fallback=fallback, context=context
                )
            else:
                return operation()
    ...
    def perform_check(
            ...
            try:
                ...
                if success:
                    result = CheckResult.success(
                        check_name, "Check passed", duration_ms=int(duration * 1000)
                    )
                else:
                    result = CheckResult.failure(
                        check_name, "Check failed", duration_ms=int(duration * 1000)
                    )
        ...
        # 結果をセッション状態に追加
        self.session_state = self.state_manager.add_check_result(
        ...
        # システム状態更新
        health_status = (
            HealthStatus.HEALTHY if result.is_success() else HealthStatus.UNHEALTHY
        )
    ...
    def get_system_overview(self) -> Dict[str, Any]:
        ...
        def get_overview():
            session_overview = self.state_manager.get_system_overview(
                self.session_state
            )
            ...
            return {
                ...
                "system_health": {
                    ...
                    "unhealthy_components": [
                        comp.component_name
                        for comp in self.system_status.get_unhealthy_components()
                    ],
                },
                "configuration": {
                    "session_id": self.config.session_id,
    ...
    def validate_system_integrity(self) -> Dict[str, Any]:
        ...
        def validation_operation():
            # セッション状態の整合性検証
            state_validation = self.state_manager.validate_state_integrity(
                self.session_state
            )
            ...
            # 整合性判定
            is_healthy = (
                state_validation["is_valid"]
                and cache_stats["hit_rate"] >= 0.0  # キャッシュが機能している
                and error_stats.total_errors < 100  # エラー数が許容範囲内
            )
    ...
    def _handle_state_change_event(self, event: Union[StateChangeEvent, Any]) -> None:
        ...
        try:
            if hasattr(event, "change_type"):
                self.logger.debug(
                    f"Handling state change event: {event.change_type.value}"
                )
            elif hasattr(event, "event_type"):
