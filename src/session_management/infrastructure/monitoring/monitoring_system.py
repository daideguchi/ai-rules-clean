    ...
    def export_metrics(self) -> Dict[str, Any]:
        ...
        with self._lock:
            return {
                ...
                "metrics_count": sum(
                    len(deque_obj) for deque_obj in self.metrics_history.values()
                ),
        ...
        for rule in self.threshold_rules:
            try:
                ...
                history = metric_collector.get_metric_history(
                    rule.metric_name, rule.labels, limit=100
                )
                ...
                cutoff_time = datetime.now() - timedelta(
                    seconds=rule.evaluation_window_seconds
                )
                recent_values = [m.value for m in history if m.timestamp >= cutoff_time]
                ...
                if rule.evaluate(recent_values):
                    # 既存のアクティブアラートをチェック
                    existing_alert_key = (
                        f"{rule.metric_name}_{rule.operator}_{rule.threshold}"
                    )
                    if any(
                        a.name == existing_alert_key
                ...
                self.logger.error(
                    f"Error evaluating threshold rule {rule.metric_name}: {e}"
                )
...
class MonitoringSystem:
    ...
    def _apply_configuration(self) -> None:
        ...
        if "health_check_interval" in self.config:
            self.health_monitor.health_check_interval = self.config[
