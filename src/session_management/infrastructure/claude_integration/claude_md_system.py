...
class CLAUDEMdParser:
    ...
    def _extract_rules(
        ...
        for section in sections:
            ...
            enforcement_level = self._determine_enforcement_level(
    ...
    def _determine_rule_category(self, title: str) -> RuleCategory:
        ...
        if any(
            ...
            for keyword in ["president", "重要", "critical", "必須"]
        ):
            return RuleCategory.CRITICAL
        elif any(
            keyword in title_lower
            for keyword in ["mandatory", "応答", "template", "format"]
        ):
            return RuleCategory.MANDATORY
        elif any(
            keyword in title_lower
            for keyword in ["workflow", "手順", "procedure", "process"]
        ):
            return RuleCategory.WORKFLOW
        elif any(
            keyword in title_lower for keyword in ["mcp", "integration", "統合", "ai"]
        ):
    ...
    def _determine_rule_type(self, content: str) -> RuleType:
        ...
        if any(
            ...
            for keyword in ["bash", "command", "make", "python"]
        ):
            return RuleType.COMMAND_EXECUTION
        elif any(
            keyword in content_lower
            for keyword in ["file", "path", "directory", "folder"]
        ):
            return RuleType.FILE_OPERATION
        elif any(
            keyword in content_lower
            for keyword in ["response", "応答", "format", "template"]
        ):
            return RuleType.RESPONSE_FORMAT
        elif any(
            keyword in content_lower
            for keyword in ["check", "validate", "verify", "確認"]
        ):
            return RuleType.VALIDATION_CHECK
        elif any(
            keyword in content_lower
            for keyword in ["mcp", "integration", "統合", "system"]
        ):
            return RuleType.SYSTEM_INTEGRATION
        else:
            return RuleType.VALIDATION_CHECK

    def _determine_enforcement_level(
        ...
        if any(
            keyword in text
            for keyword in ["critical", "必須", "mandatory", "禁止", "block"]
        ):
    ...
    def _extract_commands(self, content: str) -> List[str]:
        ...
        commands.extend(
            ...
            if any(word in cmd for word in ["make", "python", "bash", "cd", "ls"])
        )
...
class RuleEnforcementEngine:
            ...
            if rule.category == RuleCategory.CRITICAL:
                for required_cmd in rule.commands:
                    if required_cmd in [
                        ...
                        "python3 scripts/hooks/critical_failure_prevention.py",
                    ]:
    ...
    def check_file_operation(
        self, file_path: str, operation: str, context: Dict[str, Any]
    ) -> List[RuleViolation]:
        """ファイル操作チェック"""
        violations = []
            ...
            return {
                ...
                "violations_summary": {
                    ...
                    "active_violations": len(
                        [v for v in self.violations_history if not v.resolved]
                    ),
                ...
                "recent_violations": [
                    v.to_dict() for v in self.violations_history[-10:]
                ],
            }
...
class CLAUDEMdIntegrationSystem:
    ...
    def _log_violation(self, violation: RuleViolation) -> None:
        """違反ログ出力"""
        level = (
            logging.CRITICAL if violation.severity == "CRITICAL" else logging.WARNING
        )
    ...
    def start_auto_reload(self, interval_seconds: int = 60) -> None:
        ...
        self.reload_thread = threading.Thread(
            target=self._auto_reload_loop, args=(interval_seconds,), daemon=True
        )
    ...
    def check_file_operation(
        ...
        """ファイル操作チェック"""
        return self.enforcement_engine.check_file_operation(
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_parse_time": self.last_parse_time.isoformat()
            if self.last_parse_time
            else None,
            ...
            "active_violations_count": len(
                self.enforcement_engine.get_active_violations()
            ),
