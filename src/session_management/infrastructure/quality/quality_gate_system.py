...
class QualityCheckResult:
    ...
    def complete_with_success(
        self, output: str = "", metrics: Optional[Dict[str, Any]] = None
    ):
        """成功完了"""
    ...
    def complete_with_failure(
        self, output: str = "", error_output: str = "", exit_code: int = 1
    ):
        """失敗完了"""
        self.end_time = datetime.now()
...
class QualityChecker:
        ...
        try:
            ...
            if metrics.get("total_issues", 0) == 0:
                ...
                result.complete_with_success(
                    f"Linting issues found: {metrics['total_issues']}", metrics
                )
    ...
    def _parse_pytest_output(self, output: str) -> Dict[str, Any]:
        ...
        metrics["total_tests"] = (
            metrics["passed"] + metrics["failed"] + metrics["errors"]
        )
        ...
        try:
            ...
            if coverage_file.exists():
                with open(coverage_file) as f:
                    ...
                    metrics["coverage_percentage"] = coverage_data.get(
                        "totals", {}
...
class QualityGateSystem:
    ...
    def _execute_checks_sequential(self) -> Dict[QualityCheckType, QualityCheckResult]:
        ...
        for check_type in self.config.enabled_checks:
            if self.config.fail_fast and any(
                not r.is_success() for r in results.values()
            ):
                break
        ...
        for check_type, result in results.items():
            if result.metrics:
                if check_type == QualityCheckType.COVERAGE:
                    metrics.test_coverage_percentage = result.metrics.get(
        ...
        if self.config.policy == GatePolicy.STRICT:
            ...
            critical_passed = all(
                results.get(
                    check_type,
                    QualityCheckResult(
                        check_type, QualityStatus.FAILED, datetime.now()
                    ),
                ).is_success()
            ...
            # 最低限のテストがパスすれば合格
            test_passed = results.get(
                QualityCheckType.UNIT_TESTS,
                QualityCheckResult(
                    QualityCheckType.UNIT_TESTS, QualityStatus.FAILED, datetime.now()
                ),
            ).is_success()
    ...
    def _apply_custom_rules(
        self,
        results: Dict[QualityCheckType, QualityCheckResult],
        metrics: QualityMetrics,
    ) -> bool:
        """カスタムルール適用"""
        rules_passed = []
    ...
    def _generate_summary(
        self, results: Dict[QualityCheckType, QualityCheckResult], overall_passed: bool
    ) -> Dict[str, Any]:
        """サマリー生成"""
        ...
        error_count = sum(
            1 for result in results.values() if result.status == QualityStatus.ERROR
        )
