        ...
        try:
            ...
            async with self.session.post(
                ...
                if response.status == 200:
                    ...
                    self.logger.info(
                        f"Message sent successfully to {message.recipient.value}"
                    )
...
class CollaborationOrchestrator:
    ...
    def _integrate_model_responses(
        self, responses: Dict[str, Any], task: CollaborationTask
    ) -> Dict[str, Any]:
        """モデル応答統合"""
        integrated = {
            "task_id": task.task_id,
        ...
        if AIModel.GEMINI.value in responses:
            ...
            integrated["recommendations"].extend(
                gemini_response.get("recommendations", [])
            )
    ...
    def _generate_consensus(self, responses: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if recommendation_counts:
            consensus["final_recommendation"] = max(
                recommendation_counts, key=recommendation_counts.get
            )
    ...
    def _setup_endpoints(self) -> None:
        ...
        for model_name, endpoint_config in endpoints_config.items():
            try:
                ...
                self.logger.error(
                    f"Invalid endpoint configuration for {model_name}: {e}"
                )

    def _handle_incoming_message(self, message: MCPMessage) -> None:
        """受信メッセージ処理"""
        self.logger.info(
            f"Received message from {message.sender.value}: {message.message_type.value}"
        )
    ...
    async def collaborate_on_task(
        ...
        try:
            ...
            task = self.orchestrator.create_collaboration_task(
                title=title, description=description, pattern=pattern, context=context
            )
