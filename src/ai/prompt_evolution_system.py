...
class PromptEvolutionSystem:
    ...
    def __init__(self, project_root: str = "/Users/dd/Desktop/1_dev/coding-rule2"):
        ...
        self.evolution_log_path = (
            self.project_root / "runtime" / "logs" / "prompt_evolution.log"
        )
    ...
    def _generate_success_based_changes(
    ...
    def _generate_failure_based_changes(
        self, pattern_data: Dict
    ) -> List[EvolutionChange]:
        """失敗パターンベースの変更生成"""
        changes = []
    ...
    def apply_evolution_changes(self, changes: List[EvolutionChange]) -> bool:
        ...
        try:
            ...
            for change in changes:
                ...
                self.logger.info(
                    f"Applied change: {change.change_type} - {change.rule_id}"
                )
            ...
            if self._validate_evolved_content(content):
                ...
                self.logger.info(
                    f"Successfully applied {len(changes)} evolution changes"
                )
                return True
            else:
        ...
        if change.change_type == "add":
            ...
            if change.old_content:
                ...
                content = self._insert_after_section(
                    content, "## 🔴 最優先必須", strengthening_section
                )
    ...
    def _validate_evolved_content(self, content: str) -> bool:
        """進化後コンテンツ検証"""
        # 基本構造チェック
        required_sections = ["## 🔴 最優先必須", "CLAUDE.md", "記憶継承システム"]
    ...
    def _create_backup(self) -> bool:
        ...
        try:
            ...
            if self.claude_md_path.exists():
                with open(self.claude_md_path, encoding="utf-8") as src:
    ...
    def _restore_from_backup(self) -> bool:
        ...
        try:
            ...
            with open(latest_backup, encoding="utf-8") as src:
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        if command == "evolve":
            ...
            print(
                f"   Success Patterns: {result['patterns_analyzed']['success_patterns']}"
            )
            print(
                f"   Failure Patterns: {result['patterns_analyzed']['failure_patterns']}"
            )
            ...
            for i, pattern in enumerate(patterns["success_patterns"][:5]):
                print(
                    f"   Top Success #{i + 1}: {pattern['patterns']} (freq: {pattern['frequency']})"
                )
