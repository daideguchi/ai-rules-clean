-- AI自律成長システム用 Supabaseテーブル設定
-- 以下のSQLをSupabase Dashboard > SQL Editorで実行
...
-- 基本的なポリシー（必要に応じて調整）
CREATE POLICY "Allow all operations for authenticated users" ON ai_performance_log
    FOR ALL USING (auth.role() = 'authenticated');

CREATE POLICY "Allow all operations for authenticated users" ON ai_learning_patterns
    FOR ALL USING (auth.role() = 'authenticated');

CREATE POLICY "Allow all operations for authenticated users" ON claude_evolution_history
    FOR ALL USING (auth.role() = 'authenticated');

-- サービスロール用ポリシー（n8nアクセス用）
CREATE POLICY "Allow service role full access" ON ai_performance_log
    FOR ALL USING (auth.role() = 'service_role');

CREATE POLICY "Allow service role full access" ON ai_learning_patterns
    FOR ALL USING (auth.role() = 'service_role');

CREATE POLICY "Allow service role full access" ON claude_evolution_history
    FOR ALL USING (auth.role() = 'service_role');
